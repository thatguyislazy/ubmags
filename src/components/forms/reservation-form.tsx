...
export function ReservationForm({
  ...
  function toErrorMessage(err: unknown): string {
    ...
    // Common shape from Zod's `error.flatten()`
    const maybe = err as {
      formErrors?: unknown;
    ...
    const fieldErrors = maybe.fieldErrors && typeof maybe.fieldErrors === "object" ? maybe.fieldErrors : undefined;
    if (fieldErrors) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/resources?category=EQUIPMENT").then((r) => r.json()),
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Client-side checks for clearer feedback than generic "Invalid input"
    ...
    const equipmentIds = equipment
      .filter((eq) => form.get(`eq-${eq.id}`) === "on")
      .map((eq) => ({
        resourceId: eq.id,
        quantity: (() => {
          const raw = parseInt(String(form.get(`eq-qty-${eq.id}`) || "1"), 10);
    ...
    if (!res.ok) {
      if (typeof window !== "undefined") {
        // Helps debugging when API returns Zod flatten() object
        // (shows which field is invalid in DevTools console)
        console.warn("[MAGS] reservation submit failed", { status: res.status, data });
      }
      setError(
        data?.conflicts
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <section className="space-y-3">
          ...
          {venues.map((v) => (
            <label
              ...
              className={`flex cursor-pointer items-start gap-2 rounded-lg border p-3 transition-colors ${
                selectedVenues.includes(v.id) ? "border-ub-maroon bg-ub-maroon/5" : "border-gray-200"
              }`}
            >
              <input
                type="checkbox"
        ...
        <div className="space-y-2">
          ...
          <Input id="customVenueSpecify" name="customVenueSpecify" placeholder="Custom venue specification" />
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {equipment.map((eq) => (
            <label key={eq.id} className="flex items-center gap-2 rounded-lg border p-3 text-sm">
              <input type="checkbox" name={`eq-${eq.id}`} />
              {eq.name}
              <Input name={`eq-qty-${eq.id}`} type="number" min={1} defaultValue={1} className="ml-auto w-16" />
            </label>
          ))}
        </div>
      </section>
      ...
      <div className="space-y-2">
        ...
        <Input id="conformeName" name="conformeName" placeholder="Name for conforme signature block" />
      </div>
