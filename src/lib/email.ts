...
export async function sendTransactionalEmail(params: {
  ...
  console.log(`[MAGS Email] Subject: ${subject}`);
  console.log(`[MAGS Email] Using Resend API`);

  if (!process.env.RESEND_API_KEY) {
    console.log(`[MAGS Email] ⚠️ RESEND_API_KEY not configured`);
    return { sent: false, error: "RESEND_API_KEY not configured" };
  }

  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'MAGS System <onboarding@resend.dev>',
      to: [to],
      subject,
      html,
    ...
    console.log(`[MAGS Email] ✅✅✅ Email sent successfully to ${to}: ${data?.id}`);
    return { sent: true };
  } catch (e) {
...
function getReservationEmailTemplate(status: string, reservation: any) {
  ...
  if (hasEquipment && reservation.equipment) {
    itemsList = reservation.equipment.map((e: any) => 
      `<li>${e.resource.name} - Quantity: ${e.quantity}</li>`
    ).join('');
  } else if (hasVenues && reservation.venues) {
    itemsList = reservation.venues.map((v: any) => 
      `<li>${v.resource.name}${v.specifyText ? ` (${v.specifyText})` : ''}</li>`
    ).join('');
  }

  const baseUrl = process.env.NEXTAUTH_URL || 'https://ubmags-production.up.railway.app';
...
export async function sendReservationStatusEmail(
  ...
  console.log(`[MAGS Email] Recipient: ${to}`);
  console.log(`[MAGS Email] Status: ${status}`);
  ...
  console.log(`[MAGS Email] Email template generated - Subject: ${subject}`);
  ...
  console.log(`[MAGS Email] Result: sent=${result.sent}, error=${result.error || 'none'}`);
  return result;
}
